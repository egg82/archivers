...
echo "Starting gov_archiver.."

if [ -z "$TAG" ]; then
  export TAG="gov"
fi

if [ -z "$DOMAIN_TYPE_NEGATIVE_FILTER_REGEX" ]; then
  export DOMAIN_TYPE_NEGATIVE_FILTER_REGEX="^(City|County|State|Interstate|School|Special|Tribal)"
fi
